# Template Smoke Fixtures

Use these paste-ready fixtures in **Import conversation / notes** to spot-check non-Praxis templates. Praxis remains the primary full manual smoke-test fixture in `docs/manual-smoke-test.md`.

## Software App

```text
Raw idea: Build a local-first software app that turns messy notes into project plans with workflow, UI states, persistence, validation, and export.
...
```text
Raw idea: Create a business system for a small service offer that helps gym owners improve client onboarding, retention, and proof of value.
...
```text
Raw idea: Build a coaching system for assessing clients and adapting training plans through intake, assessment, intervention, feedback, adaptation, and safety boundaries.
...
```text
Raw idea: Create a content and brand framework around alignment, structure, fitness, and AI-assisted building.
...
```text
Raw idea: Write a white paper about governed AI collaboration and structural accountability.
...
```text
Raw idea: Create a repeatable SOP workflow for reviewing Codex output before merging.
...
```text
Raw idea: Create a general decision framework for turning raw desire into structured action without relying on a stronger domain template.
