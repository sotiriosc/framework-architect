...
const doNotBreakFor = (blueprint: ProjectBlueprint, templateId: FrameworkTemplateId): string[] =>
  unique([
    ...
    "Do not rewrite the whole app or replace user-authored architecture wholesale.",
  ...
  "generic-framework": [
    {
      ...
      description: "Build the smallest workflow that proves the blueprint can produce useful implementation guidance.",
      phase: "foundation",
      priority: "must",
      terms: ["mvp", "foundation", "intent", "workflow"],
      expectedFiles: ["core workflow module", "MVP UI", "MVP tests"],
      acceptanceCriteria: ["A user can complete the first workflow.", "Output stays connected to outcomes.", "Validation remains visible."],
      suggestedTests: ["Run unit tests for core workflow.", "Run npm run test."],
      riskNotes: ["Do not build future automation before the MVP path is useful."],
      doNotTouch: ["Backend, auth, billing, or external AI unless explicitly in scope"],
      estimatedScope: "medium",
    },
    {
      key: "governance-export",
      title: "Add governance and export handoff",
      description: "Preserve rules, invariants, validation, and export outputs through the first implementation path.",
      phase: "export",
      priority: "should",
      terms: ["governance", "validation", "export", "handoff"],
      expectedFiles: ["validation module", "export utility", "Codex prompt export"],
      acceptanceCriteria: ["Governance remains inspectable.", "Export output is deterministic.", "MVP and expansion remain distinct."],
      suggestedTests: ["Run validation tests.", "Run export tests."],
      riskNotes: ["Export text should not silently add future scope."],
